...
try {
    ...
    while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
        ...
        $row['child_triase'] = fetchChildData($pdo, 'asesmentriase_t', 'td_systolic, td_diastolic, detaknadi, pernapasan, suhutubuh, tinggibadan_cm, beratbadan_kg, spo2 AS spo, CONCAT(TRIM(COALESCE(CAST(gcs_eye AS CHAR(10)), \'\')), \', \', TRIM(COALESCE(CAST(gcs_verbal AS CHAR(10)), \'\')), \', \', TRIM(COALESCE(CAST(gcs_motorik AS CHAR(10)), \'\'))) AS gcs_nilai, lingkar_kepala, lingkar_lengan, lingkar_dada', 'pegawai_m', 'pegawai_id', $pendaftaran_id);
