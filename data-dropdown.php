...
try {
    ...
    $sql = "SELECT pegawai_id as pelapor_id, nama_pegawai  AS name  FROM pegawai_m"; // Sesuaikan dengan tabel dan kolom Anda
    if ($query) {
        // Tambahkan kondisi WHERE jika ada parameter pencarian
        $sql .= " WHERE nama_pegawai ILIKE :search"; // ILIKE mendukung pencarian case-insensitive
    }
